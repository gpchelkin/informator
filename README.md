...
Можно поменять либо вручную в базе, либо указать в качестве параметра метода `Notice.load_file` в `config/initializers/informator_loader.rb`.
...
Применить такой подход? **UPD:** Реализовал, с Markdown.
